  ...
  return {
    ...
    resolveTtsConfig: vi.fn(
      (
        ...
        mode: string;
        progress?: { durableStatus: "off" | "immediate"; livePreview: "off" | "immediate" };
      } => ({ mode: "final" }),
    ),
  };
});
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("adds an audio-only supplement for durable plan updates when progress TTS is immediate", async () => {
    ...
    const cfg = {
      ...emptyConfig,
      messages: {
        ...emptyConfig.messages,
        tts: { progress: { durableStatus: "immediate", livePreview: "off" } },
      },
      agents: {
        defaults: {
          verboseDefault: "on",
  ...
  it("keeps durable plan update text when immediate progress TTS fails", async () => {
    ...
    expect(dispatcher.sendToolResult).toHaveBeenCalledWith(
      expect.objectContaining({ text: "Inspect code.", isStatusNotice: true }),
    );
  });

  it("does not synthesize partial or reasoning previews under durable progress TTS", async () => {
    setNoAbort();
    ttsMocks.state.synthesizeFinalAudio = true;
    ttsMocks.resolveTtsConfig.mockReturnValue({
      mode: "final",
    ...
    const replyResolver = async (_ctx: MsgContext, opts?: GetReplyOptions) => {
      await opts?.onPartialReply?.({ text: "draft preview" });
      await opts?.onReasoningStream?.({ text: "reasoning preview" });
      return undefined;
    };
